...
function log(step, detail) { const line = detail ? step + ': ' + detail : step; evidence.push(line); console.log('[e2e] ' + line) }
...
async function go(path) { await page.goto(new URL(path, BASE).toString(), { waitUntil: 'domcontentloaded' }); await ready() }
async function field(label, value) { await page.getByText(label, { exact: true }).locator('..').locator('input,textarea').first().fill(value) }
async function snap(name) { await page.screenshot({ path: 'e2e-artifacts/' + name + '.png', fullPage: true }).catch(() => {}) }
...
async function registerAndOnboard() {
  ...
  await inputs.nth(0).fill(service); await inputs.nth(1).fill('120'); await inputs.nth(2).fill('60')
...
async function ensureService() {
  ...
  if (await page.getByText(service, { exact: true }).count()) {
    log('serviço onboarding', 'presente em Configurações')
    return
  }
  log('serviço onboarding', 'AUSENTE; fallback por Configurações')
  const section = page.getByText('Adicionar serviço', { exact: true }).locator('..')
  const inputs = section.locator('input')
  await inputs.nth(0).fill(service)
  await inputs.nth(2).fill('120')
  await inputs.nth(3).fill('60')
  await section.getByRole('button', { name: 'Adicionar serviço', exact: true }).click()
  await page.getByText(service, { exact: true }).waitFor({ timeout: 15000 })
  log('serviço fallback', 'criado por Configurações')
}
...
async function createClient() {
  ...
  await page.waitForURL(/\/crm\/[0-9a-f-]+$/, { timeout: 30000 }); await ready(); clientUrl = page.url()
...
async function wellnessBasics() {
  ...
  await field('Nome do pacote', pack); await field('Sessões', '1'); await field('Valor pago', '100'); await field('Observações', 'Pacote E2E')
  ...
  await field('Local da dor/desconforto', 'Trapézio e cervical'); await field('Intensidade da dor (0–10)', '6')
  await field('Condições de saúde', 'Nenhuma informada no teste.'); await field('Medicamentos em uso', 'Nenhum')
  await field('Alergias', 'Nenhuma'); await field('Contraindicações/cuidados', 'Nenhuma')
  await field('Cirurgias/procedimentos prévios', 'Nenhuma'); await field('Observações gerais', 'Anamnese E2E.')
  ...
  for (let i = 0; i < 2; i++) {
    ...
    await page.getByText('Consentimento registrado.', { exact: true }).waitFor({ timeout: 15000 }); await page.waitForTimeout(300)
  }
...
async function appointment() {
  await go('/booking'); await page.getByRole('button', { name: 'Novo agendamento', exact: true }).click()
  const modal = page.locator('h2').filter({ hasText: 'Novo agendamento' }).locator('..')
  const serviceSelect = modal.getByText('Serviço *', { exact: true }).locator('..').locator('select')
  const serviceId = await serviceSelect.locator('option').filter({ hasText: service }).first().getAttribute('value')
  assert.ok(serviceId, 'service missing from booking selector')
  ...
  const selects = date.locator('select'); assert.ok(await selects.count() >= 2); await selects.nth(0).selectOption('16'); await selects.nth(1).selectOption('00')
  const clientSelect = modal.getByText('Cliente', { exact: true }).locator('..').locator('select')
  const clientId = await clientSelect.locator('option').filter({ hasText: client }).first().getAttribute('value'); assert.ok(clientId); await clientSelect.selectOption(clientId)
  ...
  await detail.getByRole('button', { name: 'concluído', exact: true }).click(); await page.waitForTimeout(500)
  const cls = await detail.getByRole('button', { name: 'concluído', exact: true }).getAttribute('class'); assert.ok(cls && cls.includes('bg-amber-100'))
...
async function evolutionAndConsume() {
  await page.goto(clientUrl, { waitUntil: 'domcontentloaded' }); await ready()
  ...
  const id = await appt.locator('option').filter({ hasText: service }).first().getAttribute('value'); assert.ok(id); await appt.selectOption(id)
  await field('Escala de dor (0–10)', '4'); await field('Regiões trabalhadas', 'trapézio, cervical'); await field('Técnicas utilizadas', 'deslizamento, pressão')
  await field('Evolução do atendimento', 'Cliente apresentou redução de tensão e melhora do desconforto.')
  await field('Resposta do cliente', 'Relatou alívio ao final da sessão.'); await field('Recomendações', 'Hidratação e alongamento leve.')
  await page.getByRole('button', { name: 'Salvar evolução' }).click()
  await page.getByText('Evolução registrada.', { exact: true }).waitFor({ timeout: 15000 }); log('evolução', 'registrada')
  await page.getByRole('button', { name: 'Pacotes', exact: true }).click()
  ...
  await page.getByText(/1\/1 sessões utilizadas · 0 restantes/).waitFor(); log('pacote', 'sessão consumida; saldo 0')
}

async function cleanup() {
  if (!clientUrl) return
  await page.goto(clientUrl, { waitUntil: 'domcontentloaded' }).catch(() => {}); await ready().catch(() => {})
  const trash = page.locator('button:has(svg.lucide-trash-2)').first()
  if (await trash.count()) {
    await trash.click(); const confirm = page.getByRole('button', { name: 'Excluir cliente', exact: true })
    if (await confirm.count()) { await confirm.click(); await page.waitForURL(u => u.pathname === '/crm', { timeout: 15000 }).catch(() => {}); log('cleanup', 'cliente E2E excluído') }
  }
}

try {
  await registerAndOnboard(); await ensureService(); await createClient(); await wellnessBasics(); await appointment(); await evolutionAndConsume()
  await snap('success'); console.log('E2E_RESULT=PASS'); console.log(evidence.join('\n'))
} catch (e) {
  await snap('failure'); console.error('E2E_RESULT=FAIL'); console.error(e && e.stack ? e.stack : e); throw e
} finally {
  await cleanup().catch(e => console.error('[e2e] cleanup error', e)); await context.close().catch(() => {}); await browser.close().catch(() => {})
}
