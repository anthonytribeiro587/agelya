...
const consumeNew = "  const select = page.locator('select').filter({ hasText: 'Selecionar atendimento concluído' }).first()\n  const option = select.locator('option').filter({ hasText: service }).first()\n  const useId = await option.getAttribute('value')\n  assert.ok(useId, 'completed appointment missing from package selector')\n  await select.selectOption(useId)\n  await page.getByRole('button', { name: 'Consumir sessão', exact: true }).click()"
...
const generated = new URL('./.generated-massotherapy-v4.mjs', import.meta.url)
